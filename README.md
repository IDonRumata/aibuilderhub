...
| 🤖 **Autonomous content** | Daily news pipeline in `automation/` — see [automation/BRIEF.md](automation/BRIEF.md) |
| 🔒 **Security headers** | CSP, HSTS, frame-deny via `public/_headers` — see [SECURITY-AUDIT.md](SECURITY-AUDIT.md) |
| 📡 **RSS feed** | `/rss.xml` generated by `@astrojs/rss` |
...
| Layer | Tool | Why |
|-------|------|-----|
| **Framework** | [Astro 7.x](https://astro.build) | Zero JS by default, best static site SEO |
| **Styling** | [Tailwind CSS 3.x](https://tailwindcss.com) via PostCSS | Utility-first, no CSS files to manage |
...
A daily pipeline in `automation/` collects AI news from public feeds and APIs,
writes one post in the site's voice, runs it past four independent reviewer
agents, strips machine-writing tells, and commits the result. It runs on
GitHub Actions at 06:30 UTC and can be triggered manually with
`workflow_dispatch`.

```bash
cd automation
uv sync
uv run aibh-pipeline topics       # what would be written about, no LLM calls
uv run aibh-pipeline run --draft  # full run, published as a draft
```

Required repository secret: `ANTHROPIC_API_KEY`. Optional: `VOYAGE_API_KEY`.
Full documentation: [automation/BRIEF.md](automation/BRIEF.md).
